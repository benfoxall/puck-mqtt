...
const debug = require('debug')('puck-mqtt:bridge')
...
class Bridge {
  ...
  constructor(device, mqtt_host) {
    ...
    client.on('message', (topic, message) => {
      debug('from MQTT:', topic, message.toString())
      const payload = `if(this.MQTT_handle) this.MQTT_handle( atob('${btoa(topic)}'),atob('${btoa(message.toString())}'));\n`
      ble.send(payload)
    })
...
class BLE {
  ...
  connect() {
    ...
    .then(() => {
      ...
      debug("resolved queue", allMessages)
    })
  }
